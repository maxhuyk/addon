...
class SleepyBaby:
    ...
    def process_baby_image_models(self, img, debug_img):
        ...
        if results_pose.pose_landmarks:
            ...
            debug_img = cv2.putText(
                debug_img,
                "Left wrist",
                (int(left_wrist_coords[0]), int(left_wrist_coords[1])),
                2,
                1,
                (255, 0, 0),
                2,
            ...
            debug_img = cv2.putText(
                debug_img,
                "Right wrist",
                (int(right_wrist_coords[0]), int(right_wrist_coords[1])),
                2,
                1,
                (255, 0, 0),
                2,
        ...
        if results.multi_face_landmarks:
            ...
            if eyes_are_open == 0:  # if eyes are closed, then check if mouth is open
                mouth_is_open = check_mouth_open(
                    results.multi_face_landmarks[0].landmark
                )
                if mouth_is_open:
                    logging.info(
                        "Eyes closed, mouth open, crying or yawning, consider awake."
                    )
    ...
    def need_to_clean_this_up(self, wake_status, img):
        ...
        if wake_status:  # woke up
            ...
            with open(
                sleep_data_base_path + "/sleep_logs.csv", "a+", encoding="utf-8"
            ) as f:
                f.write(log_string)
            cv2.imwrite(
                p, img
            )  # store off image of when wake/sleep event occurred. Can help with debugging issues
    ...
    def awake_voting_logic(self, debug_img):
        if (
            len(self.eyes_open_q) > len(self.eyes_open_q) / 2
        ):  # dont vote on eyes unless queue is half full
    ...
    def movement_voting_logic(self, debug_img, body_found):
        if not body_found:
            ...
            avg_std = (
                ((std_left_wrist_x + std_left_wrist_y) / 2)
                + ((std_right_wrist_x + std_right_wrist_y) / 2)
            ) / 2
    ...
    def frame_logic(self, raw_img):
        ...
        if os.getenv("DEBUG", "False").lower() in ("true", "1"):
            ...
            adj_avg_awake = 1.0 if avg_awake / 0.6 >= 1.0 else avg_awake / 0.6
            progress_end_point = (
                int(w / 2 - bar_width / 2 + (bar_width * (adj_avg_awake))),
                370 + bar_y_offset,
            )
            ...
            debug_img = cv2.rectangle(
                debug_img, start_point, end_point, color, thickness
            )
            debug_img = cv2.rectangle(
                debug_img, start_point, progress_end_point, progress_color, thickness
            )
            ...
            debug_img = cv2.putText(
                debug_img,
                "Awake",
                (int(w / 2 - bar_width / 2 + 85), 330 + bar_y_offset),
                2,
                1,
                (255, 0, 0),
                2,
                ...
                if cur_time > self.next_frame:
                    ...
                    if all(e is not None for e in [frame, img]):
                        ...
                        if img.shape[0] > 1080 and img.shape[1] > 1920:  # max res 1080p
                            img = maintain_aspect_ratio_resize(
                        ...
                        if os.getenv("DEBUG", "False").lower() in ("true", "1"):
                            ...
                            cv2.putText(
                                img,
                                text,
                                (int(img.shape[0] / 2) + 250, int(img.shape[1] / 2)),
                                2,
                                3,
                                text_color,
                                2,
            ...
            if len(consumer_q) > 0:
                ...
                if img.shape[0] > 1080 and img.shape[1] > 1920:  # max res 1080p
                    img = maintain_aspect_ratio_resize(
                        self=self,
                        image=img,
                        width=self.frame_dim[0],
                        height=self.frame_dim[1],
                    )
                    ...
                    try:
                        ...
                        for face_landmarks in self.multi_face_landmarks:
                            ...
                            self.mpDraw.draw_landmarks(
                                ...
                                landmark_drawing_spec=None,
                                connection_drawing_spec=self.mpDraw.DrawingSpec(
                                    color=(255, 150, 255), thickness=1, circle_radius=1
                                ),
                            )
                            # connection_drawing_spec=self.mpDrawStyles
                            # .get_default_face_mesh_contours_style())
...
def receive(producer_q):
    ...
    ] = "rtsp_transport;tcp"  # Use tcp instead of udp if stream is unstable
